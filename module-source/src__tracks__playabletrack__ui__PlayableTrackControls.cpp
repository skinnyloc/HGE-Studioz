...
namespace {
   ...
   constexpr auto ArmButtonHeight = 20;
   constexpr auto ArmButtonExtra = 2;
...
void GetArmButtonBounds( const wxRect & rect, wxRect &dest )
{
   dest = rect;
}
...
{
   wxCoord textWidth, textHeight;
   const auto armed = HgeTrackArm::IsArmed(pTrack);
   const auto selected = pTrack ? pTrack->GetSelected() : true;
   const auto str = armed ? _("ARMED") : _("Arm");

   AColor::Bevel2(*dc, armed || down, bev, selected, hit);

   TrackInfo::SetTrackInfoFont(dc);
   dc->SetTextForeground(armed ? wxColour(180, 0, 0) : wxColour(35, 35, 35));
   dc->GetTextExtent(str, &textWidth, &textHeight);
   dc->DrawText(str, bev.x + (bev.width - textWidth) / 2, bev.y + (bev.height - textHeight) / 2);
}
  ...
  const wxRect &rect, const Track *pTrack )
{
   ...
   GetArmButtonBounds( rect, bev );
   {
      auto target = dynamic_cast<ArmButtonHandle*>( context.target.get() );
      bool hit = target && target->GetTrack().get() == pTrack;
      bool captured = hit && target->IsDragging();
      bool down = captured && bev.Contains( context.lastState.GetPosition());
      ArmDrawFunction( dc, bev, pTrack, down, captured, hit );
   }
}
}
...
void PlayableTrackControls::GetArmButtonRect
(const wxRect & rect_, wxRect & dest, const Track *pTrack)
{
   const auto rect = wxRect(rect_).Deflate(CommonTrackInfo::Margin);
   ...
   auto &trackControl = static_cast<const CommonTrackControls&>(
      TrackControls::Get( *pTrack ) );
   const auto resultsA = TrackInfo::CalcItemY( trackControl.GetTCPLines(), TCPLine::kItemArm );
   GetArmButtonBounds(rect, dest);
   dest.y = rect.y + resultsA.first;
   dest.height = resultsA.second;
}
...
const TCPLines& PlayableTrackControls::StaticNoteTCPLines()
{
   ...
   std::call_once( flag, []{
      ...
      { TCPLine::kItemArm, ArmButtonHeight, ArmButtonExtra,
         ArmDrawFunction },
