...
bool ProjectFileManager::ImportIntoTrack(
   wxArrayString fileNames, WaveTrack &targetTrack, double importTime,
   bool addToHistory)
{
   fileNames.Sort(FileNames::CompareNoCase);
   ...
   std::set<Track*> beforeTracks;
   for (auto track : tracks)
      beforeTracks.insert(track);

   bool importedAny = false;
   for (const auto &fileName : fileNames) {
      wxLogDebug("HGE targeted drop import file=%s target=%p time=%f",
         fileName, &targetTrack, importTime);
      if (!DoImport(fileName, addToHistory, nullptr))
         return false;
      importedAny = true;
   }
   ...
   std::vector<WaveTrack*> importedTracks;
   for (auto track : tracks)
      if (!beforeTracks.count(track))
         if (auto waveTrack = dynamic_cast<WaveTrack*>(track))
            importedTracks.push_back(waveTrack);

   if (importedTracks.empty()) {
      wxLogDebug("HGE targeted drop found no imported wave tracks; keeping fallback import");
      return true;
   }
   ...
   for (auto importedTrack : importedTracks) {
      ...
      const auto dstChannels = targetTrack.NChannels();
      if (srcChannels == 1 || srcChannels == dstChannels) {
         targetTrack.Paste(importTime, *importedTrack);
         tracks.Remove(*importedTrack);
         movedIntoTarget = true;
         wxLogDebug(
            "HGE targeted drop pasted imported track=%p into target=%p srcChannels=%zu dstChannels=%zu",
            importedTrack, &targetTrack, srcChannels, dstChannels);
      }
      else {
         wxLogDebug(
            "HGE targeted drop kept imported track=%p due channel mismatch srcChannels=%zu dstChannels=%zu",
            importedTrack, srcChannels, dstChannels);
      }
   }

   if (movedIntoTarget) {
      SelectUtilities::SelectNone(project);
      targetTrack.SetSelected(true);
      TrackFocus::Get(project).Set(&targetTrack, true);
      history.PushState(XO("Imported audio into '%s'").Format(targetTrack.GetName()),
         XO("Import"));
      viewport.HandleResize();
      trackPanel.Refresh(false);
   }
