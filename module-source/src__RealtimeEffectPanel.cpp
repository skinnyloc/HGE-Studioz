...
namespace
{
   ...
   public:
      ...
      {
         wxMenu menu;
         std::map<int, wxString> hgePluginIds;
         ...
         const auto hgeMenuBuilt = Get().PopulateHgeMenu(menu, hgePluginIds);
         ...
         if(parent->PopupMenu(&menu, parent->GetClientRect().GetLeftBottom()) && commandId != wxID_NONE)
         {
            ...
            if(hgeMenuBuilt)
            {
               auto it = hgePluginIds.find(commandId);
               if(it != hgePluginIds.end())
                  return it->second;
            }
            else
   ...
   private:
      ...
      bool PopulateHgeMenu(wxMenu& menu, std::map<int, wxString>& pluginIds)
      {
         struct HgeMenuEntry
         {
            wxString id;
         ...
         auto classify = [](const PluginDescriptor& plugin) {
            ...
            const auto internal = plugin.GetSymbol().Internal();
            const auto display = plugin.GetSymbol().Msgid().Translation();
            const auto path = plugin.GetPath();
            const auto haystack = (internal + wxT(" ") + display + wxT(" ") + path).Lower();

            if (haystack.Contains(wxT("tdr nova")))
               return Result{ wxT("EQ"), wxT("TDR EQ"), 1, true, true };
            if (haystack.Contains(wxT("ssq")))
               return Result{ wxT("EQ"), wxT("SSQ EQ"), 2, true, true };
            if (haystack.Contains(wxT("pteq")))
               return Result{ wxT("EQ"), wxT("PTEq-X"), 3, true, true };

            if (haystack.Contains(wxT("tdr kotelnikov")))
               return Result{ wxT("Compressors"), wxT("TDR Compressor"), 1, true, true };
            if (haystack.Contains(wxT("compressor")))
               return Result{ wxT("Compressors"), wxT("Compressor"), 10, !plugin.IsEffectDefault(), false };
            ...
            if (haystack.Contains(wxT("loudness")) || haystack.Contains(wxT("lufs")) ||
                haystack.Contains(wxT("master")) || haystack.Contains(wxT("equalizer")))
               return Result{ wxT("Mastering"), display, 1, !plugin.IsEffectDefault(), false };

            if (haystack.Contains(wxT("meter")) || haystack.Contains(wxT("analy")) ||
                haystack.Contains(wxT("rms")) || haystack.Contains(wxT("cleanup")))
               return Result{ wxT("Utility"), display, 1, !plugin.IsEffectDefault(), false };

            return Result{
               wxT("Utility"),
               display.empty() ? internal : display,
               100,
               !plugin.IsEffectDefault(),
               false
            };
         };
         ...
         for (const auto& plugin : PluginManager::Get().Plugins(
            [](const PluginDescriptor& desc) {
               return
                  desc.GetPluginType() == PluginTypeEffect &&
                  desc.GetEffectType() == EffectTypeProcess &&
                  desc.IsEnabled() &&
                  desc.IsEffectRealtime();
            }))
         {
            if (PluginManager::Get().IsPluginLoaded(plugin.GetID()) &&
                EffectManager::Get().IsHidden(plugin.GetID()))
               continue;

            const auto info = classify(plugin);
            if (!info.visible)
               continue;
         ...
         for (const auto& category : categoryOrder)
         {
            ...
            std::sort(entries.begin(), entries.end(), [](const auto& a, const auto& b) {
               return std::make_tuple(a.sortOrder, a.label, a.id) <
                      std::make_tuple(b.sortOrder, b.label, b.id);
            });
            ...
            for (const auto& entry : entries)
            {
               ...
               if (entry.certified)
                  label += wxT("  HGE Certified");
               submenu->Append(commandId, label);
               pluginIds[commandId] = entry.id;
               ++commandId;
            }

            menu.AppendSubMenu(submenu.release(), category);
            hasItems = true;
         }
